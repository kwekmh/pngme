...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> crate::Result<Chunk> {
        let mut idx = 0;
        let mut found = false;
        let mut res: crate::Result<Chunk> = Err("No such chunk to remove".into());
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        self.header.iter()
            .copied()
            .chain(self.chunks().iter().map(|c| c.as_bytes()).flatten())
            .collect()
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        while idx < value.len() {
            let data_length = u32::from_be_bytes((value[idx..(idx + 4)]).try_into().unwrap());
            if data_length <= value[idx..].len() as u32 - 4 - 4 - 4 {
                let total_length = 4 + 4 + data_length + 4;
                let chunk = Chunk::try_from(&value[idx..(idx + total_length as usize)])?;
                chunks.push(chunk);
                idx += total_length as usize;
            } else {
...
impl fmt::Display for Png {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let all_bytes: Vec<u8> = self.header.iter()
            .copied()
            .chain(self.chunks.iter().map(|c| c.as_bytes()).flatten())
            .collect();
